...
from database import carregar_tabela, conn
...
@media (max-width: 640px) {
    .block-container { padding: 0.75rem 0.65rem 2rem !important; }
    .finance-header { gap: 9px; padding: 0.8rem 0 0.7rem; margin-bottom: 0.9rem; }
    .finance-logo { width: 36px; height: 36px; border-radius: 10px; }
    .finance-title { font-size: 1.05rem; }
    .finance-sub { font-size: 0.64rem; }
    div[data-testid="stMetric"] { padding: 0.8rem !important; border-radius: 12px !important; }
    div[data-testid="stMetricValue"] { font-size: 1.12rem !important; }
    .stTabs [data-baseweb="tab-list"] { overflow-x: auto; border-radius: 10px; }
    .stTabs [data-baseweb="tab"] { font-size: 0.7rem !important; padding: 7px 10px !important; white-space: nowrap; }
    .form-panel { padding: 1rem; border-radius: 12px; }
    .parcela-card { padding: 0.75rem 0.85rem; }
}
</style>
...
#  DADOS
# ──────────────────────────────────────────────
df = carregar_tabela("lancamentos")
parcelas = carregar_tabela("parcelas")
        ...
        if not parcelas.empty:
            ...
            <div class="form-panel">
              ...
              <div style="display:flex;justify-content:space-between;margin-bottom:8px">
                ...
                <span style="font-family:'JetBrains Mono',monospace;color:{COLOR_ALERTA};font-size:0.88rem;font-weight:700">
                    R$ {ab:,.2f}
                </span>
              </div>
              ...
              <div style="display:flex;justify-content:space-between">
                ...
                <span style="font-family:'JetBrains Mono',monospace;color:{COLOR_ENTRADA};font-size:0.88rem;font-weight:700">
                    R$ {pg:,.2f}
                </span>
              </div>
            </div>
        ...
        if not parcelas.empty:
            ...
            for _, r in p_view.iterrows():
                ...
                with bc1:
                    ...
                    if st.button(
                        "✓ Marcar Pago" if not pago else "↩ Reabrir",
                        key=f"st_{rid}",
                ...
                with bc2:
                    ...
                    if st.button(
                        ...
                        st.session_state[f"edit_parc_{rid}"] = not st.session_state.get(
                            f"edit_parc_{rid}",
                            False
                        )
                ...
                if st.session_state.get(f"edit_parc_{rid}", False):
                    ...
                    with st.container():
                        ...
                        nova_data = st.date_input(
                            "Nova Data",
                            value=pd.to_datetime(r["vencimento"]).date(),
                            key=f"dt_{rid}"
                        )
                        ...
                        with csave:
                            ...
                            if st.button(
                                ...
                                st.session_state[f"edit_parc_{rid}"] = False
                                st.rerun()
...
with tab_admin:
    ...
    if not df.empty:
        ...
        # DATA
        hist = hist[
            (hist["data"] >= pd.Timestamp(data_inicio)) &
            (hist["data"] <= pd.Timestamp(data_fim))
        ]
        ...
        # FORMATAÇÃO
        hist["data_br"] = hist["data"].dt.strftime("%d/%m/%Y")
        ...
        <div style="
            ...
            color:#7F8C8D;
        ">
            {len(hist_show)} registro(s) encontrado(s)
        </div>
        """, unsafe_allow_html=True)
